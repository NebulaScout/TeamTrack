...
class TaskViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):  # type: ignore
        ...
        return (
            TaskModel.objects.filter(Q(created_by=user) | Q(assigned_to=user))
            .distinct()
    ...
    def update_status(self, request, pk=None):
        ...
        update_task = TaskService.update_task_status(
            task_id=task.id, user=request.user, status=new_status
        )
    ...
    def update_priority(self, request, pk=None):
        ...
        update_task = TaskService.update_task_priority(
            task_id=task.id, user=request.user, priority=new_priority
        )
    ...
    def comments(self, request, pk=None):
        ...
        if request.method == "POST":
            ...
            comments = CommentModel.objects.filter(task=task).select_related("author")
