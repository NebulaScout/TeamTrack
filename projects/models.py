...
from core.services.enums import ProjectStatusEnum, PriorityEnum
...
class ProjectMembers(models.Model):
    ROLE_CHOICES = [
        (role, role) for role in ROLE_PERMISSIONS.keys()
    ]  # Fetch the defined permissions

    project = models.ForeignKey(
        ProjectsModel, on_delete=models.CASCADE, related_name="members"
    )  # record reprsents one user has one role in the project
    ...
    role_in_project = models.CharField(
        max_length=50, choices=ROLE_CHOICES, default="Guest"
    )
